...
// ---- Mirrors StudentFeeLedger.jsx's monthly rollover model ----
// StudentFeeLedger only "spends" a month's fee out of a student's balance
// when that student's OWN ledger page is opened (it's a lazy rollover).
// Finance reads the raw `balance` field straight from Firestore, so a
// student nobody has opened in a couple of months can still be sitting on
// a stale, un-rolled-over balance — which is what was inflating "Advance
// Balance" on this dashboard. These two helpers let Finance project what
// balance/status SHOULD be right now, without needing every student's page
...
const applyRolloverStep = (balance, fee) => {
  const f = Number(fee || 0);
  if (balance <= 0) return { balance, status: "Unpaid" };
  if (f <= 0) return { balance, status: "Advance" };
  if (balance === f) return { balance: 0, status: "Paid" };
  if (balance > f) return { balance: balance - f, status: "Advance" };
  return { balance, status: "Partial" };
};
// Read-only projection: replays every missed month's rollover in memory
// (never writes to Firestore) so dashboard totals are correct even for
// students whose ledger hasn't been opened this month.
const projectStudentState = (student) => {
  const fee = Number(student.monthlyFee || 0);
  let balance = Number(student.balance || 0);
  let status = student.feeStatus || "Unpaid";
  const thisMonth = currentMonthKey();
  const storedMonth = student.currentBillingMonth;
  if (!storedMonth || storedMonth === thisMonth) {
    return { balance, status: status || computeStatusFromBalance(balance, fee) };
  }
  let mk = storedMonth;
  let guard = 0;
  while (mk !== thisMonth && guard < 600) {
    mk = nextMonthKey(mk);
    const step = applyRolloverStep(balance, fee);
...
const Finance = ({ schoolId: schoolIdProp }) => {
  ...
  // Self-heal stale balances in the background. StudentFeeLedger only rolls
  // a student's balance forward when THAT student's own ledger page gets
  // opened, so a student nobody has looked at in a while can sit on a
  // months-old, un-rolled-over balance in Firestore. The dashboard cards
  // already read the corrected, in-memory projection (projectStudentState)
  // so they're accurate immediately — this effect additionally writes the
  // same correction back to Firestore in the background so the underlying
  // data actually catches up too. Skipped for any student who hasn't been
  // through StudentFeeLedger's one-time tuition-only migration yet
  // (balanceMigratedV2), since that repair needs their full payment
  // history and belongs to StudentFeeLedger, not this page.
  useEffect(() => {
    if (!schoolId || students.length === 0) return;
    const thisMonth = currentMonthKey();
    students.forEach((s) => {
      if (!s.balanceMigratedV2) return;
      if (!s.currentBillingMonth || s.currentBillingMonth === thisMonth) return;
      const studentRef = doc(db, "students", s.id);
      runTransaction(db, async (transaction) => {
        const snap = await transaction.get(studentRef);
        if (!snap.exists()) return;
        const data = snap.data();
        if (!data.currentBillingMonth || data.currentBillingMonth === thisMonth)
          return;
        const fee = Number(data.monthlyFee || 0);
        let balance = Number(data.balance || 0);
        let status = data.feeStatus || "Unpaid";
        let mk = data.currentBillingMonth;
        let guard = 0;
        while (mk !== thisMonth && guard < 600) {
          mk = nextMonthKey(mk);
          const step = applyRolloverStep(balance, fee);
        ...
        transaction.update(studentRef, {
          balance,
          feeStatus: status,
          currentBillingMonth: thisMonth,
          lastRolloverAt: new Date().toISOString(),
        });
      }).catch((err) =>
        console.error(`Error rolling over student ${s.id}:`, err),
      );
    });
  ...
  // Every one of these three cards is built from the SAME projected state
  // per student (projectStudentState), so Paid/Partial/Advance/Unpaid
  // counts, Outstanding Receivables, and Advance Balance always agree with
  // each other instead of drifting apart.
  const projectedStudents = useMemo(
    () =>
      filteredStudents.map((s) => ({ ...s, ...projectStudentState(s) })),
    [filteredStudents],
  );
  ...
  // Outstanding Receivables = how much of this month's fee is still owed
  // across Unpaid + Partial students — it's directly the flip side of the
  // status counts above.
  const outstandingReceivables = useMemo(
    () =>
      projectedStudents.reduce((sum, s) => {
    ...
    <div className="space-y-6 w-full">
              ...
              <p className="text-[10px] text-rose-400 mt-1">
                {isClassFiltered ? (
                  <>
                    ...
                    <br />
                    Operational overhead excluded — switch to All Classes to
                    see it
                  </>
                ) : (
                  <>
      ...
      {isFeeModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md p-5 relative z-60">
              ...
              {editingFeeEntry ? (
                ...
                <>
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      {feeFormClassId === "all"
                        ? "Showing students from every class."
                        : `Showing students from ${classNameMap[feeFormClassId] || "this class"} only.`}
                    </p>
                  </div>
