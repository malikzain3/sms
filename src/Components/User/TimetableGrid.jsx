  ...
  <div className="flex items-center justify-center gap-2 mt-1.5 flex-wrap">
    ...
    {onCopyMonSat && (
      <button
        ...
        className="px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-300 rounded text-[10px] font-bold flex items-center gap-1 transition cursor-pointer shadow-xs whitespace-nowrap"
      >
        ⚡ Copy Mon-Sat
      </button>
    )}
...
const selectClass =
  "w-full p-1.5 bg-white text-slate-900 border border-indigo-500 rounded text-center text-xs focus:outline-none shadow-sm";
const linkSelectClass =
  "w-full p-1.5 bg-white text-slate-700 border border-slate-300 rounded text-[11px] focus:outline-none";
...
const CellEditor = ({
  ...
  const SpanSelector = (
    <div className="flex items-center justify-between gap-1.5 text-[10px] font-bold text-slate-500 bg-slate-50 p-1.5 rounded-lg border border-slate-200 mt-0.5">
 ...
 const handleResetDefaultSlots = () => {
  if (
    ...
    const defaultSlots = [
      ...
      "09:20 - 10:00",
      "10:00 - 10:40",
      "10:40 - 11:00 (Break)", // Breakdown/Break label added here
      "11:00 - 11:40",
      "11:40 - 12:20",
      "12:20 - 01:00",
    ];
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xl w-full text-xs text-slate-800 space-y-3">
          ...
          {readOnly ? (
            ...
            <>
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition cursor-pointer shadow-sm ${
                  isEditingStructure
              ...
              >
                {isEditingStructure ? (
                  <span className="flex items-center gap-1.5 text-xs">
                    <Save size={14} /> Lock Structure
                  </span>
                ) : (
                  <span className="flex items-center gap-1.5 text-xs">
                    <SquarePen size={14} /> Edit Structure & Rows
                  </span>
                )}
              </button>
              {isEditingStructure && (
                <>
                  <button
                    ...
                    className="px-2 py-1 bg-indigo-600 text-white rounded-lg text-[10px] font-bold hover:bg-indigo-700 cursor-pointer shadow-sm transition-all"
                  >
                    + Add Time Row
                  </button>
                  <button
                    ...
                    className="px-2 py-1 bg-emerald-600 text-white rounded-lg text-[10px] font-bold hover:bg-emerald-700 cursor-pointer shadow-sm transition-all"
                  >
                    + Add Day Col
                  </button>
                  <button
                    ...
                    className="px-2 py-1 bg-rose-500 text-white rounded-lg text-[10px] font-bold hover:bg-rose-700 cursor-pointer shadow-sm transition-all"
                  >
        ...
        <table className="w-full min-w-187.5 border-collapse text-center">
          <thead>
            <tr className="bg-slate-100 border-b border-slate-200">
              <th className="p-2 w-36 text-[11px] text-slate-600 font-bold border-r border-slate-200">
                Time Interval Range
              </th>
              {days.map((day) => (
                <th
                  key={day}
                  className="p-2 font-bold border-r border-slate-200 last:border-r-0 relative text-slate-800 min-w-40 text-sm"
                >
                  <span>{day}</span>
                  {isEditingStructure && (
                    <button
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <td
                        ...
                        className={`p-2 border-r border-slate-200 last:border-r-0 font-medium text-[10px] transition select-none align-middle ${
                          isGlobalBreak
