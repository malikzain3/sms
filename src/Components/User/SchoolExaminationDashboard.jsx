 ...
 * Teacher Resolver
 * -------------------------------------------------------------------------
 * Given the class/section/subject picked in the exam form, figure out who
 * the timetable already says should teach it, so the admin doesn't have to
 * re-look-up the teacher every time. Two schedule shapes exist on a class
 * doc (see TimetableGrid.jsx):
 *   - scheduleType === "class"  -> one homeroom teacher covers every period.
 *     The teacher's *name* lives at classDoc.teachers[0].
 *   - scheduleType === "period" (default) -> each timetableMatrix entry can
 *     carry its own { subject, teacherId }. We match on subject (trimmed,
 *     case-insensitive) and take the first entry that has a teacherId.
 * Returns a teacherId or "" if nothing could be resolved (e.g. the class
 * doesn't have a timetable configured yet) — callers should treat that as
 * "no recommendation available", not an error.
 * ---------------------------------------------------------------------- */
const resolveRecommendedTeacherId = (
  classesData,
  teachersData,
  className,
  section,
  ...
  const classDoc = classesData.find(
    (c) => (c.className || c.name) === className && c.section === section,
  );
  if (!classDoc) return "";

  // Class-based schedule: the single homeroom/class teacher handles this
  // subject too — resolved by name since that's how TimetableGrid links it.
  if (classDoc.scheduleType === "class") {
    const homeroomName = Array.isArray(classDoc.teachers)
      ? classDoc.teachers[0]
      : null;
    if (!homeroomName) return "";
    const match = teachersData.find((t) => t.name === homeroomName);
    return match?.id || "";
  }

  // Period-based schedule: find the timetable slot teaching this subject.
  if (!subject) return "";
  const subNorm = subject.trim().toLowerCase();
  const matrix = Array.isArray(classDoc.timetableMatrix)
    ? classDoc.timetableMatrix
    : [];
  const slot = matrix.find(
    (entry) =>
      entry?.teacherId &&
      (entry.subject || "").toString().trim().toLowerCase() === subNorm,
  );
  ...
  return (
    ...
    <div className="space-y-6 w-full animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-md sm:text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[15px] font-bold shadow-md shadow-indigo-500/20 transition hover:-translate-y-0.5 shrink-0"
        >
          <Plus className="w-5 h-5" /> Create Examination
        </button>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-3 border-t border-slate-100">
            ...
            {activeFilterCount > 0 && (
              <button
                ...
                className="col-span-2 sm:col-span-3 lg:col-span-6 text-[14px] font-bold text-rose-500 hover:text-rose-600 text-left mt-1"
              >
                Clear all filters
              </button>
            )}
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="bg-slate-50 text-slate-400 font-bold uppercase text-11px] border-b border-slate-100">
...
const FilterSelect = ({ label, value, onChange, options }) => (
  <label className="block">
    ...
    <select
      ...
      className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-2 text-[13px] font-bold text-slate-700 focus:outline-none focus:border-indigo-500 transition"
    >
    ...
    <tr className="hover:bg-slate-50/60 transition align-middle">
      <td className="p-4">
        ...
        <div className="flex flex-wrap items-center gap-1 mt-1">
          {assignedTeacher && (
            <span className="inline-block text-[11px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">
              Teacher: {assignedTeacher.name}
            </span>
          )}
          {(exam.isTeacherCreated ||
            exam.creationType === "teacher_initiated") && (
            <span className="inline-flex items-center gap-1 text-[11px] font-black uppercase tracking-wider text-purple-700 bg-purple-50 border border-purple-200 px-2 py-0.5 rounded-md">
              <Star className="w-2.5 h-2.5" /> Teacher-Initiated Test
            </span>
          )}
        </div>
      </td>
      ...
      <td className="p-4 text-[14px]">
        ...
        {exam.submissionDeadline ? (
          <p className="text-[12px] text-rose-500 font-bold flex items-center gap-1 mt-0.5">
            <Clock className="w-3 h-3" /> Deadline:{" "}
            {new Date(exam.submissionDeadline).toLocaleDateString()}
          </p>
        ) : (
          <p className="text-[12px] text-slate-400">No deadline set</p>
        )}
      </td>
      ...
      <td className="p-4">
        {exam.paperLink ? (
          <button
            type="button"
            onClick={() => openDocumentInNewTab(exam.paperLink)}
            className="inline-flex items-center gap-1.5 text-[13px] font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1 rounded-lg transition"
          >
      ...
      <td className="p-4">
        <span
          className={`inline-flex items-center gap-1.5 text-[12px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider border ${meta.badge}`}
        >
...
const ExamFormModal = ({
  ...
  // Paper attachment: client-side Base64 conversion instead of a Firebase
  // Storage upload (Storage was causing CORS errors here). The compressed
  // data URL is written straight into `form.paperLink`, exactly like a
  // pasted URL would be — the rest of the app (ExamRow,
  // TeacherExaminationDashboard) keeps working unchanged, since it's still
  // just a string on the Firestore document.
  const [paperFileName, setPaperFileName] = useState("");
  const [paperProcessing, setPaperProcessing] = useState(false);
  const [paperError, setPaperError] = useState("");

  // Recommended teacher, resolved from the school's timetable data. Kept
  // in sync with the current class/section/subject combo so the "Subject
  // Teacher Selected" hint and highlighted option always reflect the
  // latest selection.
  const recommendedTeacherId = useMemo(
    () =>
      resolveRecommendedTeacherId(
        classesData,
        teachersData,
        form.className,
        form.section,
    ...
    [classesData, teachersData, form.className, form.section, form.subject],
  );

  useEffect(() => {
    if (initial) return; // don't clobber an existing exam's assignment
    if (recommendedTeacherId && !form.assignedTeacherId) {
      setForm((f) => ({ ...f, assignedTeacherId: recommendedTeacherId }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [recommendedTeacherId]);

  // Subject selector changes are handled explicitly (rather than only via
  // the effect above) so the Assigned Teacher field re-syncs immediately —
  // including when editing an existing examination, where the effect above
  // intentionally stays out of the way to avoid clobbering a saved
  // assignment on load.
  const handleSubjectChange = (e) => {
    const newSubject = e.target.value;
    const matchedTeacherId = resolveRecommendedTeacherId(
      classesData,
      teachersData,
      form.className,
      form.section,
    ...
    setForm((f) => ({
      ...f,
      subject: newSubject,
      assignedTeacherId: matchedTeacherId,
    }));
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setTouched(true);
    if (!isValid || paperProcessing) return;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-5 space-y-4">
          <FormField label="Examination Title" required className="text-[15px]">
            <input
              value={form.name}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <FormField label="Assigned Teacher">
              ...
              >
                ...
                {recommendedTeacherId &&
                  (() => {
                    const rec = teachersData.find(
                      (t) => t.id === recommendedTeacherId,
                    );
                    return rec ? (
                      <option key={rec.id} value={rec.id}>
                        {rec.name} (Subject Teacher)
                      </option>
                    ) : null;
                  })()}
                {teachersData
              ...
              {recommendedTeacherId && (
                <p className="text-[10px] text-indigo-500 font-semibold mt-1 flex items-center gap-1">
                  <Star className="w-3 h-3 fill-indigo-500" /> Subject Teacher Selected. You may select another teacher if needed.
                </p>
              )}
            </FormField>
          </div>
          ...
          <FormField label="Question Paper (PDF or Image)">
            <label
              className={`flex items-center gap-2.5 w-full px-3.5 py-2.5 rounded-xl border border-dashed transition ${
                paperProcessing
                  ? "border-slate-200 bg-slate-50 text-slate-400 cursor-wait"
                  : "border-slate-300 bg-slate-50 text-xs font-semibold text-slate-500 hover:border-indigo-400 hover:text-indigo-600 cursor-pointer"
              }`}
            >
              {paperProcessing ? (
              ...
              <span className="truncate">
                {paperProcessing
                  ...
                  : paperFileName
                    ? paperFileName
                    : form.paperLink
                      ? "Replace attached paper"
                      : "Choose PDF or PNG/JPG..."}
              </span>
              <input
                type="file"
                accept="application/pdf,image/png,image/jpeg"
                onChange={handlePaperFileChange}
                disabled={paperProcessing}
                className="hidden"
              />
            </label>
            {form.paperLink && !paperFileName && (
              <button
                type="button"
                onClick={() => openDocumentInNewTab(form.paperLink)}
                className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-600 mt-1.5 hover:text-indigo-800"
              >
                <FileText className="w-3 h-3" /> View currently attached
                paper
              </button>
            )}
            {paperError && (
        ...
        <div className="flex items-center justify-end gap-2 p-5 border-t border-slate-100 sticky bottom-0 bg-white">
          ...
          <button
            type="submit"
            disabled={saving || paperProcessing}
            className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 shadow-md shadow-indigo-500/20 transition"
          >
            {paperProcessing && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
            {paperProcessing
              ? "Processing paper..."
              : saving
                ? "Saving..."
...
const FormField = ({ label, required, children }) => (
  <label className="block">
    <span className="text-[13px] font-bold text-slate-500 uppercase tracking-wider block mb-1.5">
      {label} {required && <span className="text-rose-400 text-[15px]">*</span>}
    </span>
    {children}
  </label>
);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative bg-white w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto p-6 space-y-4">
        ...
        <div className="overflow-x-auto border border-slate-100 rounded-xl">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50 text-slate-400 font-bold uppercase text-[12px] border-b border-slate-100">
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
              {classStudents.map((student) => {
                ...
                return (
                  <tr key={student.id} className="hover:bg-slate-50 transition text-[15px]">
                    <td className="p-3 font-mono font-bold">
                      {student.rollNumber || "—"}
                    </td>
                    <td className="p-3 font-bold text-slate-900 text-[15px] ">
                      {student.name}
                    </td>
                    <td className="p-3">
                      <span className="text-[15px] font-bold px-2 py-0.5 rounded-md bg-slate-100 ">
                    ...
                    <td className="p-3">
                      {result?.grade ? (
                        <span className="text-[13px] font-black px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">
                          {result.grade}
                        </span>
                      ) : (
                        "—"
                    ...
                    <td className="p-3">
                      {result?.answerSheetUrl ? (
                        <button
                          type="button"
                          onClick={() =>
                            openDocumentInNewTab(result.answerSheetUrl)
                          }
                          title="View Answer Sheet"
                          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-[13px] font-bold transition"
                        >
                    ...
                    <td className="p-3 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-[13px] font-bold transition"
                      >
